...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!industry) {
    return {
      title: "Industry not found - SaaSKevin",
      robots: { index: false, follow: false },
    }
  }

  const title = `Custom Domains for ${industry.name} | SaaSKevin`
  ...
  return {
    ...
    description,
    alternates: {
      canonical: `${SITE_URL}/use-cases/industry/${slug}`,
    },
    keywords: [
...
export default async function IndustryPage({ params }: PageProps) {
  ...
  const useCases = await getUseCasesByIndustry(slug)
      ...
      },
      {
        ...
        itemListElement: [
          ...
          },
          {
            ...
            position: 3,
            name: industry.name,
            item: `${SITE_URL}/use-cases/industry/${slug}`,
          },
        ],
      },
    ],
        ...
        <div className="mx-auto max-w-6xl px-6">
          <div className="flex flex-col items-center text-center">
            ...
            <div className="inline-flex items-center gap-2 rounded-full bg-secondary px-4 py-1.5 text-sm text-muted-foreground mb-8 border border-border">
              <span>{industry.count} Use Cases</span>
            </div>
          ...
          {useCases.length === 0 && (
            <div className="text-center py-12">
              <p className="text-muted-foreground">
                No use cases found in this industry yet. Check back soon!
              </p>
            </div>
          )}
        </div>
                ...
                {industries
                  ...
                  .map((otherIndustry) => (
                    <Link
                      ...
                      href={`/use-cases/industry/${otherIndustry.slug}`}
